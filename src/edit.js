...
export default function Edit({attributes, setAttributes}) {
	const { formId, nameLabel, emailLabel, messageLabel, messageRows, buttonText, buttonBackgroundColor, buttonTextColor, nameLabelColor, emailLabelColor, messageLabelColor, nameBackground, emailBackground, messageBackground, formBackgroundColor, nameTextColor, emailTextColor, messageTextColor, formMaxWidth, formMaxWidthUnit, formMinWidth, formMinWidthUnit, buttonWidth } = attributes;
	...
	const onChangeNameLabelText = ( newNameLabel ) => {
		setAttributes( { nameLabel: newNameLabel === undefined ? 'Name' : newNameLabel } );
	}

	const onChangeEmailLabelText = ( newEmailLabel ) => {
		setAttributes( { emailLabel: newEmailLabel === undefined ? 'Email' : newEmailLabel } );
	}

	const onChangeMessageLabelText = ( newMessageLabel ) => {
		setAttributes( { messageLabel: newMessageLabel === undefined ? 'Message' : newMessageLabel } );
	}

	const onChangeMessageRows = ( newMessageRows ) => {
		setAttributes( { messageRows: newMessageRows === undefined ? 5 : newMessageRows } );
	}
	...
	return (
		<>
				...
				<PanelColorSettings
					...
					colorSettings={ [
						...
						},
						{
							...
							label: __( 'Email field Background Color', 'my-contact-form-block' )
						},
						{
							value: messageBackground,
							onChange: onChangeMessageBackground,
							label: __( 'Message field Background Color', 'my-contact-form-block' )
						},
						{
			...
			<div {...blockProps} style={{ maxWidth: formMaxWidth, minWidth: formMinWidth }}>
				<form className={SSCB_CF+'--editor'} id={ formId } style={{ backgroundColor: formBackgroundColor }}>
					<div className={SSCB_CF_group}>
						...
						/>
						<input className={SSCB_CF_input} type="text" id="name" name="name" style={{ backgroundColor: nameBackground, color: nameTextColor }}/>
					</div>
					<div className={SSCB_CF_group}>
						<RichText
							tagName="label"
						...
						/>
						<input className={SSCB_CF_input} type="email" id="email" name="email" style={{ backgroundColor: emailBackground, color: emailTextColor }}/>
					</div>
					<div className={SSCB_CF_group}>
						<RichText
							tagName="label"
						...
						/>
						<textarea className={SSCB_CF_textarea} rows={ messageRows } id="message" name="message" style={{ backgroundColor: messageBackground, color: messageTextColor }}></textarea>
					</div>
					...
					<RichText
						...
						style={ { backgroundColor: buttonBackgroundColor, color: buttonTextColor , width: buttonWidth } }
					/>
				</form>
			</div>
		</>
